...
import sys
sys.path.append(r"C:\Users\heman\AppData\Local\Packages\PythonSoftwareFoundation.Python.3.13_qbz5n2kfra8p0\LocalCache\local-packages\Python313\site-packages")
...
with col1:
    ...
    if st.button("Explore LULC Analysis →", use_container_width=True, type="primary"):
...
with col2:
    ...
    if st.button("Explore AQI Analysis →", use_container_width=True, type="primary"):
...
with col3:
    ...
    if st.button("Explore Heat Analysis →", use_container_width=True, type="primary"):
...
with col4:
    ...
    if st.button("Explore Prediction →", use_container_width=True, type="primary"):
...
with data_col1:
    st.markdown(source_style.format(title="Sentinel-2", desc="10m Optical • 5-day Revisit"), unsafe_allow_html=True)
with data_col2:
    st.markdown(source_style.format(title="Landsat 8/9", desc="30m Thermal • 16-day Revisit"), unsafe_allow_html=True)
with data_col3:
    st.markdown(source_style.format(title="Sentinel-5P", desc="Air Quality • Daily Global"), unsafe_allow_html=True)
with data_col4:
    st.markdown(source_style.format(title="MODIS", desc="LST & Climate • Daily"), unsafe_allow_html=True)

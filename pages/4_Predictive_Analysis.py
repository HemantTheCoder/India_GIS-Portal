...
from services.gee_core import (
    auto_initialize_gee, get_city_geometry, process_shapefile_upload, 
...
render_page_header("🔮 AI Predictive Analysis", "Forecast environmental trends using Machine Learning")
...
def get_ndvi_series(roi, start_date, end_date):
    ...
    data = collection.mean().reduceRegion(reducer=ee.Reducer.mean(), geometry=roi, scale=1000) # Quick check
    ...
    def reduce_region(image):
        ...
        return ee.Feature(None, {'date': image.date().format('YYYY-MM-dd'), 'value': mean.get('NDVI')})

    data = collection.filter(ee.Filter.calendarRange(1, 12, 'month')).map(reduce_region).filter(ee.Filter.notNull(['value'])).sort('date')
    info = data.getInfo()
    return pd.DataFrame([f['properties'] for f in features] if (features := info['features']) else [])
...
def get_aqi_series(roi, start_date, end_date, pollutant):
    ...
    config = {
        'NO2': {'id': 'COPERNICUS/S5P/OFFL/L3_NO2', 'band': 'tropospheric_NO2_column_number_density', 'scale': 1e6}, # mol/m^2 to umol/m^2 approx
        'CO': {'id': 'COPERNICUS/S5P/OFFL/L3_CO', 'band': 'CO_column_number_density', 'scale': 1},
        'SO2': {'id': 'COPERNICUS/S5P/OFFL/L3_SO2', 'band': 'SO2_column_number_density', 'scale': 1e6},
        'O3': {'id': 'COPERNICUS/S5P/OFFL/L3_O3', 'band': 'O3_column_number_density', 'scale': 1},
        'Aerosol': {'id': 'COPERNICUS/S5P/OFFL/L3_AER_AI', 'band': 'absorbing_aerosol_index', 'scale': 1}
    }
    ...
    def reduce_region(image):
        ...
        val = ee.Algorithms.If(val_raw, ee.Number(val_raw).multiply(cfg['scale']), None)
    ...
    data = collection.filter(ee.Filter.calendarRange(1, 31, 'day_of_month')).map(reduce_region).filter(ee.Filter.notNull(['value'])).sort('date')
    info = data.getInfo()
    return pd.DataFrame([f['properties'] for f in features] if (features := info['features']) else [])
...
def get_lst_series(roi, start_date, end_date):
    ...
    def scale_lst(image):
        ...
        return image.addBands(val, overwrite=True).set('system:time_start', image.get('system:time_start'))
    ...
    def reduce_region(image):
        ...
        return ee.Feature(None, {'date': image.date().format('YYYY-MM-dd'), 'value': mean.get('LST_Day_1km')})

    data = collection.map(reduce_region).filter(ee.Filter.notNull(['value'])).sort('date')
    info = data.getInfo()
    return pd.DataFrame([f['properties'] for f in features] if (features := info['features']) else [])
...
def get_lulc_area_series(roi, start_year, end_year):
    ...
    idx_to_class = {
        ...
        4: 'Crops', 5: 'Shrub', 6: 'Built Area', 7: 'Bare Ground', 8: 'Snow/Ice'
    }
...
with st.sidebar:
    ...
    if location_mode == "City Selection":
        ...
        uploaded_files = st.file_uploader("Choose files", accept_multiple_files=True, key="pred_upload")
    ...
    target_category = st.selectbox("Category", ["Air Quality (AQI)", "Land Cover (LULC)", "Urban Heat (LST)", "Vegetation (NDVI)"])
    
    target_param = ""
    target_unit = ""
    
    if target_category == "Air Quality (AQI)":
        target_param = st.selectbox("Pollutant", ["NO2", "CO", "SO2", "O3", "Aerosol"])
        if target_param == "NO2" or target_param == "SO2": target_unit = "µmol/m²"
        elif target_param == "CO": target_unit = "mol/m² - Raw"
        elif target_param == "O3": target_unit = "mol/m²"
        else: target_unit = "Index"
    ...
    model_choice = st.selectbox("Algorithm", ["Random Forest", "Linear Regression"])
...
if run_btn:
    if not st.session_state.get("gee_initialized", False):
        st.error("Google Earth Engine not initialized. Please check your secrets.")
        st.stop()
    ...
    with status_container:
        st.info(f"Fetching data for {selected_city}... Estimated time: ~30 seconds")
    ...
    try:
        ...
        if target_category == "Urban Heat (LST)":
            ...
            df = get_lulc_area_series(roi, train_start_year, datetime.now().year)
        ...
        for col in value_cols:
            ...
            # Simple prep
            X, y, last_date, features_list = prepare_time_series_data(sub_df, 'date', 'value')
            
            # Train
            model_type_code = 'random_forest' if model_choice == "Random Forest" else 'linear'
            model, metrics = train_forecast_model(X, y, model_type=model_type_code)
            ...
            # Forecast
            f_df = generate_forecast(model, last_date, features_list, periods=forecast_days)
        ...
        try:
            ...
            r2_floats = [float(v.split(' ')[0]) for v in model_metrics.values()]
        ...
        if is_multi_class:
            ...
            fig = px.bar(bar_df, x="Class", y="Area (sq km)", color="Year", barmode="group",
                         title=f"Land Cover Change: {current_year} vs {target_year}",
                         text_auto='.1f',
                         color_discrete_map={str(current_year): '#3b82f6', str(target_year): '#10b981'},
            ...
            for cls in key_classes:
                if cls in df.columns:
                    ...
                    with cols[idx % 4]:
                        st.metric(cls, f"{fut:.1f} km²", f"{delta:+.1f} km² ({delta_pct:+.1f}%)")
                    idx += 1
            ...
            st.caption(f"📈 Trend Fit Confidence: {avg_r2_val:.2f} (Auto-Degree Selection). Details: {fit_str}")
            ...
            try:
                ...
                for cls in classes:
                    ...
                    pdf_metrics[cls] = {
                        'current': curr_v, 'future': fut_v, 
                        'delta': fut_v - curr_v, 'pct': ((fut_v - curr_v)/curr_v)*100 if curr_v else 0
                    }
                ...
                for _, row in final_forecast_df.iterrows():
                    f_data.append({'date': row['date'], 'Built Area': row.get('Built Area', 0), 'type': 'predicted'})
                # Add history
                for _, row in df.iterrows():
                        f_data.append({'date': row['date'], 'Built Area': row.get('Built Area', 0), 'type': 'historical'})

                from services.exports import generate_prediction_pdf_report
                pdf_bytes = generate_prediction_pdf_report({
                    'current_year': current_year, 'target_year': target_year,
                    'metrics': pdf_metrics,
                    'confidence': f"{avg_r2_val:.2f}",
            ...
            col_d1.download_button("📥 CSV Data", csv_p, "forecast.csv", "text/csv", use_container_width=True)
            
            with col_d2:
                if st.session_state.get('pred_pdf'):
                    st.download_button("📥 Download PDF", st.session_state.pred_pdf, "prediction_report.pdf", "application/pdf", use_container_width=True, key=f"dl_pred_pdf_{datetime.now().strftime('%Y%m%d%H%M%S')}")
                else:
            ...
            change_pct = ((future_val - current_val) / current_val) * 100 if current_val != 0 else 0
            ...
            col_m2.metric(f"Projection ({predict_until_year})", f"{future_val:.2f} {target_unit}", f"{change_pct:.1f}%")
            ...
            try:
                ...
                for _, row in hist_df.iterrows():
                        f_data.append({'date': row['date'], title: row['value'], 'type': 'historical'})
                for _, row in pred_df.iterrows():
                        f_data.append({'date': row['date'], title: row['predicted_value'], 'type': 'predicted'})
                        
                from services.exports import generate_prediction_pdf_report
                pdf_bytes_s = generate_prediction_pdf_report({
                    'current_year': pd.to_datetime(hist_df['date'].max()).year,
                    'target_year': predict_until_year,
                    'metrics': {title: {'current': current_val, 'future': future_val, 'delta': future_val-current_val, 'pct': change_pct}},
                    'confidence': f"{avg_r2:.2f}",
                    'forecast_data': f_data
                })
                if pdf_bytes_s:
            ...
            with col_d2:
                if st.session_state.get('pred_pdf_s'):
                    st.download_button("📥 Download PDF", st.session_state.pred_pdf_s, "prediction_report.pdf", "application/pdf", use_container_width=True, key=f"dl_pred_pdf_s_{datetime.now().strftime('%Y%m%d%H%M%S')}")
                else:
                    st.caption("PDF generating...")

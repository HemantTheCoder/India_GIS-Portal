...
URBAN SUSTAINABILITY SCORE (USS): {scores['total_uss']:.1f}/100
...
1. VEGETATION & LAND USE
   Score: {scores['vegetation']['score']:.1f}/25 (Grade {scores['vegetation']['grade']})
   NDVI: {metrics['ndvi']:.3f}
   Impervious Surface: {metrics['impervious']*100:.1f}%

2. AIR QUALITY
   Score: {scores['aqi']['score']:.1f}/25 (Grade {scores['aqi']['grade']})
   AQI: {metrics['aqi']:.0f}
   PM2.5: {metrics['pm25']:.1f} µg/m³

3. URBAN HEAT
   Score: {scores['heat']['score']:.1f}/25 (Grade {scores['heat']['grade']})
   Land Surface Temperature: {metrics['lst']:.1f}°C

4. FUTURE RISK
   Score: {scores['prediction']['score']:.1f}/25 (Grade {scores['prediction']['grade']})
   Risk Index: {metrics['risk']:.2f}

================================================================================
                         KEY FINDINGS
================================================================================

Strongest Sector: {report['strongest_sector']}
Weakest Sector: {report['weakest_sector']} (Priority Focus Area)

================================================================================
                         RECOMMENDATIONS
================================================================================

Priority interventions for {report['weakest_sector']}:
...
{chr(10).join([f"{p['phase']}: {', '.join(p['actions'])} (Expected gain: +{p['expected_gain']} USS)" for p in report['text_sections']['roadmap']])}

================================================================================
                         METHODOLOGY
================================================================================

This report integrates satellite-derived data from:
- Sentinel-2 (10m resolution) for vegetation indices
- Dynamic World for land use/land cover classification
- ECMWF CAMS for air quality (PM2.5, PM10)
- MODIS Terra/Aqua for Land Surface Temperature

Scoring methodology based on:
- Vegetation Score: NDVI + Impervious surface ratio
- AQI Score: PM2.5-derived Air Quality Index (EPA methodology)
- Heat Score: Land Surface Temperature deviation from comfort threshold
- Prediction Score: 5-year trend analysis for environmental risk

================================================================================
                         DISCLAIMER
================================================================================

This report is generated using satellite remote sensing data and automated
analysis. Results should be verified with ground-truth data for policy
decisions. Data sources include Google Earth Engine, Copernicus, and NASA.

Report generated by India GIS & Remote Sensing Portal
================================================================================
"""
    ...
    with col_dl1:
        st.download_button(
            "📄 Download Report (TXT)",
            data=report_text,
            file_name=f"Sustainability_Report_{report['region_name'].replace(' ', '_').replace(',', '')}_{report['year']}.txt",
            mime="text/plain",
